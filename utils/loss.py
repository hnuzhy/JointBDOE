...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, num_angles=1, wrapped=False, dynamic=False, mse_conf_thre=0):
        ...
        self.num_angles = num_angles  # the default is one orientation angle 
        self.wrapped = wrapped  # whether to use wrapped loss proposed by WHENet in lmse calculation, default False
        self.dynamic = dynamic  # whether to use dynamic loss multiple obj-probability in lmae calculation, default False
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lmse = torch.zeros(1, device=device)  # MSE loss of one orientation angles
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # orientation angle(MSE loss)
                if self.num_angles:
                    if self.dynamic:
                        # do not use self.mse_conf_thre to filter out proposals
                        if len(ps):
                            pangles = ps[:, 5 + self.nc:].sigmoid()  # sigmoid() or tanh()
                            error_ori = torch.abs(pangles - tangles[i])  # orientation error
                            ps_w = ps[:, 4].repeat(self.num_angles).reshape(-1, self.num_angles)  # N*1 --> N*self.num_angles
                            if self.wrapped:
                                error_list = torch.min(error_ori, 1.0 - error_ori)  # wrapped
                                error_list = ps_w * error_list
                                l2 = torch.linalg.norm(error_list, dim=-1)
                            else:
                                error_ori = ps_w * error_ori
                                l2 = torch.linalg.norm(error_ori, dim=-1)
                            lmse += torch.mean(l2)
                    else:
                        left_index = ps[:, 4] > self.mse_conf_thre
                        ps_left = ps[left_index]
                        if len(ps_left):
                            tangles[i] = tangles[i][left_index]
                            pangles = ps_left[:, 5 + self.nc:].sigmoid()  # sigmoid() or tanh()
                            ...
                            if self.wrapped:
                                ...
                                l2 = torch.linalg.norm(error_ori, dim=-1)  # sqrt((x0-y0)**2+...+(xn-yn)**2))
                            lmse += torch.mean(l2)
    ...
    def build_targets(self, p, targets):  
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h, orientation)
        ...
        for i in range(self.nl):
            ...
            if self.num_angles:
                tangles.append(t[:, 6:6+self.num_angles])  # one orientation angle
